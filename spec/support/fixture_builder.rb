FixtureBuilder.configure do |fixture_builder|
  ...
  fixture_builder.factory do
    ...
    Factory(:user_goal, :goal => quit_smoking, :created_at => 18.days.ago).tap do |user_goal|
      ...
      user_goal.updates.create(:status => 2, :comment => "Had a few drinks last night and just had to have a smoke. Ended up having 5 or so. Fuck this is hard.", :created_at => 1.day.ago)
    ...
    Factory(:user_goal, :goal => quit_smoking, :created_at => 8.days.ago).tap do |user_goal|
      ...
      user_goal.updates.create(:status => 10, :comment => "Had a smoke a few days ago, but now feeling reenergized to make this happen. Nothing's gonna stop me now!", :created_at => 3.hours.ago)
    end
    ...
    Factory(:user_goal, :goal => lose_weight, :created_at => 9.days.ago).tap do |user_goal|
      ...
      user_goal.updates.create(:status => 7, :comment => "Finding that having a glass or two of wine at night along with a lighter dinner is really helping.", :created_at => 1.hour.ago)
    end
    ...
    Factory(:user_goal, :goal => run, :created_at => 6.days.ago).tap do |user_goal|
      ...
      user_goal.updates.create(:status => 8, :comment => "Crap, really raining this morning. Should have gone for a run yesterday...", :created_at => 30.minutes.ago)
    end
