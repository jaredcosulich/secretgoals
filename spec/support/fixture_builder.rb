FixtureBuilder.configure do |fixture_builder|
  ...
  fixture_builder.factory do
    ...
    lose_weight_tag = Factory(:tag, :title => "lose weight")
    addiction_tag = Factory(:tag, :title => "addiction")

    smoker = Factory(:user)
    quit_smoking = Factory(:goal, :user => smoker, :title => "quit smoking before I graduate", :created_at => 18.days.ago)
    ...
    quit_smoking_2 = Factory(:goal, :user => another_smoker, :title => "quit smoking now!", :created_at => 8.days.ago)
    ...
    overweight = Factory(:user)
    lose_weight = Factory(:goal, :user => overweight, :title => "Lose 15 pounds", :created_at => 10.days.ago)
    lose_weight.updates.create(:status => 8, :comment => "Feeling good about this. Going to do my best to avoid the junk food attack during the holidays", :created_at => 9.days.ago)
    lose_weight.updates.create(:status => 5, :comment => "Ha, one christmas party and I already feel like I'm slipping.", :created_at => 7.days.ago)
    lose_weight.updates.create(:status => 6, :comment => "Ok, slipped up a little at the Christmas party, but feel like I'm back on track now.", :created_at => 6.days.ago)
    lose_weight.updates.create(:status => 8, :comment => "Went for a run this morning despite the cold. Feeling pretty good.", :created_at => 3.days.ago)
    lose_weight.updates.create(:status => 4, :comment => "Skipped my run this morning. Having a hard time getting up in the dark and the cold.", :created_at => 2.hours.ago)

    lose_weight.tags << lose_weight_tag
